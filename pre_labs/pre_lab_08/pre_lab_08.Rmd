...
**Answer** We loaded a CSV file that had 25 rows and 20 columns.
...
**Answer** The arrange line organizes the information by largest TOTAL_DIFF to smallest value. TOTAL_DIFF represents the changes in registered voters from 2020 to 2022. The select line instructs the code to sort only by County and TOTAL_DIFF. Head is asking only for the first 10 to appear.
...
**Answer** This line of code created a blank canvas to make graphs on.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** It created a bar chart that is alphabetical with the information kind of smashed onto itself. The x-axis represents county names, and the y county represents the count.
...
**Answer** A new bar chart with the same count and county name axes as the last chart, but the only difference is that the bar charts has been reordered to have the data increase by snallest total difference to largest total difference of our top 10 counties from above. It did change the county name axis to reorder (County, TOTAL_DIFF).
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** We have another bar chart with the same information, but we have a new orientation of the chart. It's now horizontal with the county name on the y-axis and count on the x-axis. It's now easier to read county names. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** We see the same bar chart as above except the gray background is gone and replaced with a white backround.
...
**Answer** I ran the code to install 'ggthemes' and load its library.
...
**Answer**  We have our same bar chart as above, but the newest change is that the background is now blue with white lines - in the style of the Economist.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** We have the same bar chart as above, but the axis names have changed to county and net change. We also have a header for the chart and a source caption.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The code made a new table that counts the number of contributions per date.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** We now have a line chart adaptation of the table above where we can see the x-axis as our date and our way axis as our total_contributions.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** We have our line chart from above, but it now includes dates from each month to better visualize the data. It also created smaller backdrop lines to follow down to the date of origin.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** We have the same line chart as above but now the dates are angled now in order to make the table easier to read.
```{r}
md_winred_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
