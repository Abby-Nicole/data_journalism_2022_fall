...
**Answer** The columns stayed the same with capitalization, spaces, and the party name next to the candidate. They are also all numbers but listed as characters.
...
**Answer** The number or order of the columns. So 2 means the second column and so on.
...
**Answer** It appears that the code is suggesting that there are 57 different <li> tags from the xml_nodset below in the HTML source. Since <li>s are within <ul>s I beliece that means it would just be one <ul> tag containing all 57 <li>s.
...
**Answer** All 57 of the entries are now in a listed format. The first 20 and 50-57 aren't committee names, but 21-49 are. The lines we don't want begin with \n. 
...
**Answer** It took our committee names and turned it into a tibble rather than just being an HTML table. The tible has a value column with the committee names and a numerical column.
