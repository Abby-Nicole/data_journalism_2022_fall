...
**Answer** The output is a tibble that contains each of the press releases' title, date, url, and text all from the directory of press releases.
...
**Answer** The unique feature lists the words and symbols in the text once. So, since "dog" was listed twice in lowercase, it was only listed once.
...
**Answer** The unnest_tokens function styles all the words in the same format and eliminates the punctuation symbols. It now lists every word.
...
**Answer** This is a bar chart showing the top 25 frequency of words from the press releases. It's not interesting because these are all very common English language words.
...
**Answer** Yes, this top 10 tibble is much more interesting because it does not contain stop words. The words are more substantial like health, support, covid, etc., the number of times each word is mentioned, and the percentage of text each word makes up among all the press release words.
...
**Answer** In 2021, there are lot of health focused words such as health, covid, etc. eith health being the most prominent at 1177. In 2022, covid is no longer in the top 10. Health is still up there but is now second with 627.
...
**Task** Run the following code and describe the results.
**Answer** The tibble being made consists of bigrams or two-word phrases. the top combination is "covid 19" with 786 appearances and a 0.86% appearance rate. There are also many similar health-related covid phrases.
...
**Task** Run the following code and describe the results.
**Answer** Covid is still discussed but the amount is much smaller. There are now other issues aside from health in the top 10 such as mental health. The tibble is still showing us the amount each phrase appears and the percent frequency.
...
**Answer** The code below generated two tibbles of top words in each year as well as the determined sentiment for each word. On its face, I feel like none of these are MASSIVELY incorrect. However, we don't know the context. For example, "trump" being considered a positive word does not acknowledge that Trump is a person that many may consider to be negative. 
