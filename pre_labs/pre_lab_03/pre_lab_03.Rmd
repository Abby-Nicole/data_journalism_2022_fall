...
**Answer**
Warning: One or more parsing issues, see `problems()` for detailsRows: 476915 Columns: 13── Column Specification
```{r}
...
**Answer**
1,640 rows; under in several row/column positions, the program is expecting answers of 1/0/T/F/TRUE/FALSE but instread there are numbers that are listed as <chr> rather than the coding language for numbers <dbr> 
```{r}
...
**Answer**
No, I see no warnings or error messages when I run the code.
```{r}
...
**Answer** "district" is <dbl> (numbers), "precinct" is <chr> (characters), & "mail" is <dbl> (numbers)
```{r}
...
**Answer**: 402345 - I would think the implication is that these votes were all cast in-person on Election Day rather since they were not mailed in
...
**Answer** 12 registrations in January of 1900. I'm assuming that January 1900 was a default date of sorts, prehaps for lacking information. In actuality, I'm assuming most of the first 10 are sort-of randomly assigned dates or incredibly elderly voters. Once er get to the 1960s, I feel more confident about living people who are registered, but it does strike me as odd that those 4 months in 1966 saw over 100+ new registrations just for it to drop off again.
```{r}
yadkin_voters %>% 
...
**Answer**
The one that raises my attention the most is "DUPLICATE" as (I assume) that means someone voted twice. Was it in the same county? Speaking of, I would assume "MOVED FROM COUNTY" and "MOVED WITHIN STATE" would mean the same thing. 
```{r}
...
**Answer** typos/misspells, capitalization, abbreviations, zip code and extended zip code
...
**Answer** it changes to x1_linenumber so it doesn't start with a number
...
**Answer**
Error in `summarise()`: ! Problem while computing `total_amount = sum(amount)`. Caused by error in `sum()`: ! invalid 'type' (character) of argument; it's listing the amount as a <chr> (character) rather than a <dbl> (number) than can be used with the verb "sum"
```{r}
# cleaning function
...
**Answer** There is one repeat donor listed twice named Derrick Hamilton
...
**Answer** It removed the extra four digits for an enture zip code and shortened all the zip codes to the standard 5 digits
...
**Answer** Assuming the question meant Conowingo and not Arnold, I see two misspellings left
...
**Answer** There's now one remaining misspelling - featuring a double "o" at the end
...
**Answer** There are no misspellings left 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
