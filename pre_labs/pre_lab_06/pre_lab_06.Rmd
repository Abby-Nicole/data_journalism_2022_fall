...
**Answer** Bend is on its own line, the percents is an issue, and Jurisdiction Wide is listed as a line of data rather than a data header. ABSENTEE has dashes in it when it should just have NA for voters and turnaround.


```{r}
camden_2022 <- read_csv("tabula-camden_county_election_results.csv") %>% 
  clean_names()
...
**Answer** Bend is still its own line, and the data set put all the absentee ballots into one precinct rather than attributing them to the proper precincts where the absentee ballots were cast. Because of this, there's still the lines in ABSENTEE and Jurisdiction Wide has NAs despite the NAs not really belonging there since it's a header.
...
**Answer** This code block has 21 rows compared to Task 6's 23 rows. It removed the Jurisdiction Wide and Bend rows.
