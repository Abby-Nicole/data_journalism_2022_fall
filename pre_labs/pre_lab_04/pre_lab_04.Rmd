...
**Answer** Yes, they all have COUNTY, YEAR, DEM, REP, LIB, GRN, UNA, & Total and each have 25 rows and 9 columns. Some of the columns are listed in a different order however.
...
**Answer** There are 24 rows which works just fine. After the joining of the population and voter data, two of the rows combined thus lowering from 25 to 24.
...
**Answer** there is no error message for task 9! It's a more informed data set.

```{r}
# join on zip - but there's a problem
# remember that the first column in a join comes from the first dataframe in the statement; order matters
...
**Answer** Prince George's county is the highest for both with 60.9035922% of 18+ Black population and 78.29988% Democrats.
...
**Answer** The highest percentages are much lower but 8 of the top counties overlap. The Democrat percentages also don't match the ranking od the 18+ Asian population as closesly as the previous table. The highest Asian 18+ county is Howard at 19.3671955% and the highest percent of Democrats is Prince George's with 78.29988%.
...
**Answer** <dbl> - representing a numerical value
...
**Answer** ZCTA5N is now a character classified as <chr>.
