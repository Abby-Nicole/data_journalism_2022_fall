...
**Answer** The code is creating a new column by dividing election_day and votes. This means the number of people who voted on election day in this jurisdiction for this candidate and divided by the total number of votes in that jurisdiction for that candidate. It creates the long decimals in the percent_election_day. The way that data is currently displayed is sorted by jurisdiction and name_raw.
...
**Answer** It adjusted the data into descending order by the percent_election_day, so candidates with the the highest percent of election day voting per candidate are first. The percent lowers as the amount of non-election day voting to total votes per candidate increases.
...
**Answer** I think so because candidates with low percentages of election day votes can utilize this layout more to learn more about their bases/voters and adjust their campaign methods accordingly. While the descending is interesting, this tibble, I think, is more helpful for improvement methods for campaigns and leads to questions about how isn't showing up on election day but still voting.
...
**Answer** str_to_lower - standardizes to make make everything lower case & str_to_title - capitalizes the first letter of each word
...
**Answer** I believe the code is sorting the individual contributions from donors into one of four categories - "Less than $100," "Between $100 and $500," "Between $501 and $2900," and "More than $2,900" based on how much money that donor contributed.
...
**Answer** Since we are mutating our data, we have to make a new variable if we want to keep this sorted and narrowed down information. Mutates don't automatically save, so it's important we create a new data set if we want to use this selected Prince George's County results.
...
**Answer:** It took the expenditure_date, which was listed as a character category rather than an actual date. The code changed that to make it categorized as <date> category and rewrote it to be formated as year-month-day in oldest to newest.
...
**Answer** The month column is categorized as <date> but it looks clearly out of sorts. The data looks as though it was sorted in in descending order by total_amount, so by making the other column "month" a bit more confusing. Since it is listed as the first date of each month, it looks like all the total_amounts happen all on one day rather than all accumulated throughout the whole month.
