...
```{r}
#remremotes::install_github(c("ropensci/tabulizerjars", "ropensci/tabulizer"))
library(tidyverse)
library(janitor)
library(tabulizer)
library(glue)
...
**Answer** It opened a table with 22 columns and 19 rows. It looks like voter roll additions and removals, but it is quite messy.
...
**Answer** It took the original datafile extracted the first table. The view verb gives us the newly cleaned new voter registrations table with 17 rows and 13 properly named columns.
...
**Task** Run the following code and describe the result.
**Answer** The same process as above except the second table was extracted rather than the first one. We can now see the cleaned file with properly named column headers.
...
**Answer** I think one of the most interesting percentages is how high the democratic percentage of "volunteer" was so much higher than Republican's. Could that mean more excitement on the Democratic side? I also raise that the UNAF voter percentages were higher in every category except in provisional ballots than Republicans. It looks like it follows the theme we sometimes see in this class with an increase in UNAF due to a shift away from Republicans.
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer** I think the most interesting percentage the Democratic removals due to criminal convictions and how comparatively high it is when you look at Republicans and Unaffiliated. I think the simple entry of voter request is really interesting too when you look at the percentages. It seems to be simply people who just want to change parties or stop voting without obvious reasons to have your name removed. Do these percentages continue to rise across the years? Is it an indication of apathy?
...
The name of the dataframe is voter_reg_page_1.
```{r}
# extract the first page and turn it into a dataframe, cleaning the column names
voter_reg_page_1 <- tables[[1]] %>% as_tibble(.name_repair = "unique") %>% clean_names()
...
**Answer** The joined datafile has 16 rows compared to 17 for June and October.
...
**Task** Run the following code. How many rows do you have compared to either the October or June dataframes?
**Answer** The joined datafile has 17 rows which is the same bumber of rows as June and October.
...
**Answer** The replications of the column names and only using "06" as a distinguisher.
...
**Answer** The bar chart is showing the various methods used to get people to register as Democratic voters, and the top method at over 60% is the ERIC report. Of the 14 methods listed, the smallest percentage of Democratic registrations goes to NVRA by Mail at a little above 45%.
...
**Answer** You can compare the new months side-by-side, but the order of the methods remains largest to smallest as you read down, so some of the methods are out of order when you look between the two.
